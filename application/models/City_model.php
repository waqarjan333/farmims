<?php
/* 
 * Generated by CRUDigniter v3.2 
 * www.crudigniter.com
 */

class City_model extends CI_Model
{
    function __construct()
    {
        parent::__construct();

        // Select All Query
        // $this->db->get('country')->result_array();
    }

    /*
     * Get brand by id
     */
    function get_brand($id)
    {
        return $this->db->get_where('brand', array('id' => $id))->row_array();
    }

    /*
     * Get all brand count
     */
    function get_all_city_count()
    {
        $this->db->where('city.status!=',DELETED);
        $query = $this->db->get('city'); 
        return $query->num_rows();
    }
    // function city_by_province_id($id){
    //     $this->db->select('city.id, city.name as city_name');
    //     $this->db->where('status', ACTIVE);
    //     $this->db->where('province_id', $id);
    //     return $this->db->get('city')->result_array();

    // }
    function province_by_country_id($id){
        // $this->db->where('city.status',DELETED);
        $this->db->select('province.id, province.name as province_name');
        $this->db->where('status', ACTIVE);
        $this->db->where('country_id', $id);
        return $this->db->get('province')->result_array();
    }
    /*
     * Get all brand
     */
    function get_all_city($limit, $start, $col, $dir)
    {
        $this->db->where('city.status!=',DELETED);
        $this->db->select('city.*, province.name as province_name');
        $this->db->join('province','province.id = city.province_id');
        $query = $this->db->limit($limit, $start)->order_by($col, $dir)->get('city'); 

        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
            return null;
        }
    }

    function city_search($limit,$start,$search,$col,$dir)
    {

        $this->db->select('city.*, province.name as province_name');
        $this->db->join('province','province.id = city.province_id');
        $query = $this
                ->db
                ->where('city.status!=',DELETED)
                // ->like('province.name',$search) 
                // ->or_like('city.name',$search) 
                ->where(" (province.name LIKE '%$search%' OR city.name LIKE '%$search%' ) ")
                ->limit($limit,$start)
                ->order_by($col,$dir)
                ->get('city');
        
       
        if($query->num_rows()>0)
        {
            return $query->result();  
        }
        else
        {
            return null;
        }
    }

    function city_search_count($search)
    {
        $this->db->select('city.*, province.name as province_name');
        $this->db->join('province','province.id = city.province_id');
        $query = $this
                ->db
                ->where('city.status!=',DELETED)
                // ->like('province.name',$search) 
                // ->or_like('city.name',$search)
                ->where(" (province.name LIKE '%$search%' OR city.name LIKE '%$search%' ) ")
                ->get('city');
    
        return $query->num_rows();
    } 

    /*
     * function to add new brand
     */
    function add_city($params)
    {
        $this->db->insert('city', $params);
        return $this->db->insert_id();
    }

    /*
     * function to update brand
     */
    function update_brand($id, $params)
    {
        $this->db->where('id', $id);
        return $this->db->update('brand', $params);
    }

    /*
     * function to delete brand
     */
    function delete_brand($id)
    {
        return $this->db->delete('brand', array('id' => $id));
    }

    function delete_city($id)
    {
        return $this->db->where('id',$id)->update('city', array('status' => DELETED));
    }

    function update_city($id,$data){
        return $this->db->where('id',$id)->update('city',$data);
    }
}
